...
deleteListButton.addEventListener('click', e => {
    ...
    /* give all the list except the selected */
    selectedListId = null;
    saveAndRender();
})
...
newListForm.addEventListener('submit', e => {
    e.preventDefault()
    /* cancel the refresh of the page because of the submit */
    const listName = newListInput.value;
    if (listName == null || listName === '') return;
    const list = createList(listName);
    newListInput.value = null; /* clear the input text */
    lists.push(list);
    saveAndRender();
});
...
newTaskForm.addEventListener('submit', e => {
    ...
    newTaskInput.value = null; /* clear the input text */
    const selectedList = lists.find(list => list.id === selectedListId);
...
const renderTasks = (selectedList) => {
    selectedList.tasks.forEach(task => {
        ...
        /* true render everything inside the template*/
